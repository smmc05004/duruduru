...
describe("buildItinerary 시간 배정 규칙", () => {
  ...
  it("날짜가 바뀐 뒤에도 항목 사이에 체류시간 + 30분 간격을 유지한다", () => {
    ...
    for (let index = 1; index < items.length; index += 1) {
      ...
      const stayHours = Number(
        /추천 체류 ([\d.]+)시간/.exec(previous.note ?? "")?.[1],
      );
      ...
      expect(minutes(current.time) - minutes(previous.time)).toBe(
        Math.round((stayHours + 0.5) * 60),
      );
    }
  });
  ...
  it("모든 목적지의 일정에서 (날짜, 시각) 조합이 유일하다", () => {
    for (const destination of destinations) {
      const slots = placedItems(buildItinerary(destination, defaultInput)).map(
...
describe("buildItinerary 운영시간 표기", () => {
  it("소수 운영시간을 17.5:00 같은 형태로 출력하지 않는다", () => {
    const notes = placedItems(buildItinerary(gyeongju, defaultInput)).map(
      (item) => item.note ?? "",
    );
  ...
  it("운영시간을 HH:MM–HH:MM 형태로 출력한다", () => {
    for (const destination of destinations) {
      for (const item of placedItems(
        buildItinerary(destination, defaultInput),
      )) {
        expect(item.note).toMatch(/운영 \d{2}:\d{2}–\d{2}:\d{2}$/);
      }
    }
    const bulguksa = placedItems(buildItinerary(gyeongju, defaultInput)).find(
      (item) => item.title === "불국사",
    );
