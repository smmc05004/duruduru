...
- 변경에 맞는 Jest 단위·통합 검증과 Playwright 화면 점검을 추가하거나 실행하고, 최소 `npm run lint`와 `npm run build`를 통과시킨다.
...
6. `npm run lint`, `npm run build`, 관련 Jest 테스트, 필요한 Playwright 화면 점검을 실행하고 결과를 기록한다.
