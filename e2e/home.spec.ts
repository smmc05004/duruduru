...
test("관심사를 고르지 않으면 제출을 막고 항목 오류를 보인다", async ({ page }) => {
  await page.goto("/");
...
test("유효한 조건을 제출하면 계산 중을 거쳐 추천 결과가 보인다", async ({ page }) => {
  await page.goto("/");
  ...
  await expect(page.getByRole("region", { name: "제출한 여행 조건" })).toBeVisible();

  await expect(page.getByText(/다녀올 수 있는 곳 \d+군데/)).toBeVisible();
  await expect(page.getByText("시간 적합순")).toBeVisible();
  // 왕복이 가장 짧은 공주가 첫 후보다.
  await expect(page.getByRole("heading", { level: 2 }).first()).toHaveText("공주");
  await expect(page.getByText("가장 잘 맞아요")).toBeVisible();

  // F-03 근거와 F-05 신뢰도 표시
  await expect(page.getByText(/쓸 수 있는 시간 중 이동에 쓰이지 않은 비율/).first()).toBeVisible();
  await expect(page.getByText(/고른 관심사 중 겹친 개수/).first()).toBeVisible();
  await expect(page.getByText(/이동시간 추정 · PoC 목업/).first()).toBeVisible();
...
test("모든 후보가 시간 제약에 걸리면 결과 없음을 정상 상태로 보인다", async ({ page }) => {
  await page.goto("/");
  ...
  await expect(page.getByRole("button", { name: "갈 수 있는 곳 찾기" })).toBeVisible();
});
