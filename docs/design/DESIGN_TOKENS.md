...
| `--ink-body` | `#3d3a2c` | 본문 |
| `--ink-muted` | `#5a5442` | 보조 정보 |
| `--ink-faint` | `#6a6450` | 각주, 고지 문구, 비활성 라벨 |
...
| `--ink-faint` | 최초값 `#8a8368`(HSL 47.6 / 14% / 47.5%)에서 **명도만 47.5% → 36.5%**로 낮춰 `#6a6450`. 색상환은 옮기지 않았다. 이유는 아래 「대비 검증」 |
| `--ink-muted` | 최초값 `#6b6550`(HSL 46.7 / 14.4% / 36.7%)에서 **명도만 36.7% → 30.6%**로 낮춰 `#5a5442`. `--ink-faint`가 옛 `--ink-muted` 자리로 내려왔기 때문에, 두 단계가 한 색으로 합쳐지지 않도록 잉크 계열 전체를 한 단 내렸다 |
...
`--ink-faint`는 각주(11px/500)와 비활성 라벨(11~14px)에 쓴다. 이 크기는 WCAG의 large text(18px+ 또는 14px bold)에 들지 않으므로 **AA 4.5:1이 적용된다.** 최초값 `#8a8368`은 세 배경 모두에서 미달이었다(paper 3.13 · card 3.58 · card-muted 3.33). 그래서 크기를 올려 large text 기준으로 피하는 대신 **명도만 낮춰 정면으로 4.5:1을 넘겼다.** 각주를 14px bold로 키우면 각주가 본문보다 굵어져 방향 H의 위계가 뒤집히고, 용도를 텍스트/비텍스트로 쪼개도 현재 `app/globals.css`의 `--ink-faint` 사용처 열 곳이 모두 텍스트여서 해결되지 않는다.

계산은 WCAG 2.x 상대 휘도 공식(sRGB 역감마 → 0.2126R+0.7152G+0.0722B, `(L1+0.05)/(L2+0.05)`)으로 직접 구한 값이다.

| 전경 | `--paper` `#efe9d5` | `--card` `#fbf8ee` | `--card-muted` `#f4f0e2` | AA(4.5:1) |
| --- | --- | --- | --- | --- |
| `--ink` `#2b2a1d` | 11.91 | 13.61 | 12.68 | 통과 |
| `--ink-body` `#3d3a2c` | 9.39 | 10.74 | 10.00 | 통과 |
| `--ink-muted` `#5a5442` | 6.21 | 7.11 | 6.62 | 통과 |
| `--ink-faint` `#6a6450` | 4.87 | 5.56 | 5.18 | 통과 |
| (옛 `--ink-faint` `#8a8368`) | 3.13 | 3.58 | 3.33 | **미달 — 쓰지 않는다** |

`--field` `#fdfcf6`는 `--card`보다 밝아 같은 전경에서 대비가 더 높다(`--ink-faint` 기준 5.75:1). 위 표의 세 배경을 통과하면 `--field` 위도 통과한다.

배경 색끼리의 조합은 텍스트가 아니라 3:1(비텍스트 대비)만 필요하다. 확인된 다른 조합도 함께 남긴다.
...
**새 잉크 색을 추가하거나 배경 표면을 바꾸면 이 표를 다시 계산해서 갱신한다.** 값을 고치고 표를 그대로 두면 다음 사람이 통과했다고 믿는다.
