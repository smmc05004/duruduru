...
| 방문 (관광지) | 6px `--olive-bar` | `--olive-ink` 11px/700 + 건물 SVG |
| 식사 (배정됨) | 6px `--olive-bar` | `--olive-ink` 11px/700 + 포크·나이프 SVG, "점심/저녁 · 음식점" |
| 식사 (비어 있음) | 없음. `--card-muted` 바탕 + 2px dashed `--line-dashed` | `--ink-muted` — `추천할 식당을 더 찾지 못했어요` |
| 축제 (P1, 미구현) | 6px `--mustard` | `--mustard-ink` 11px/700 + 별 SVG |

올리브 띠는 "여행지에서 머무는 1시간 활동"(관광 방문·식사)을 함께 뜻하고, 띠 없음은 지역 간 이동이다. 방문과 식사는 머리 라벨·아이콘으로 구분한다. 현재 MVP는 여행지 내부 장소 사이 이동을 계산·표시하지 않으므로 "장소 사이 이동" 행은 쓰지 않는다.
