...
5. 기능형 MVP 구현 기간에는 자동 검증을 실행하지 않는다. 구현 완료 후 사용자가 직접 화면을 확인한다.
6. 기능형 MVP 구현 기간에는 독립 reviewer agent 검토도 생략한다.
...
기능·화면 구현의 작업 단계와 근거는 기본적으로 `docs/agent/WORK_CYCLE.md`의 그래프와 `npm run cycle` 명령으로 추적한다. 단, `AGENTS.md`의 MVP 기능 구현 간소화 규칙이 적용되는 동안에는 사이클 기록을 생략한다.
