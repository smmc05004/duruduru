...
> **현재 상태(2026-09-11): PR #76 병합·배포 완료, 추천 품질 목표 미달. R1 진단 완료, 다음 작업은 R2다.** 아래 T1~~T7은 구현 이력이며 다시 실행할 작업이 아니다. 제품 재작업 기준은 [목적지 추천 품질 재작업 기준](../product/RECOMMENDATION_QUALITY_RECOVERY.md)이 우선한다. R1 산출물은 [R1 진단 보고서](RECOMMENDATION_QUALITY_R1_DIAGNOSIS.md)다.
...
| R2   | 미착수               | 기존 데이터로 구별 가능한 목적 적합성과 구별 불가능한 매력을 분리한다. 소수의 대안 정렬/세 후보 조합을 오프라인으로 비교한다. 결과는 `docs/development/RECOMMENDATION_QUALITY_COMPARISON.md` 하나에 입력·버전·실행 명령·전후 표·한계와 함께 작성한다. R1 보고서 §9("R2로 넘길 관찰")를 출발점으로 삼되 확정된 결론으로 취급하지 않는다.                                                                             | 기존 48개 입력 및 주요 후보의 실제 장소로 개선 근거를 확인한다. 명확한 개선이 없으면 실패를 보고하고 근거 없는 상수 튜닝을 반복하지 않는다.                                                                                                                                                                                                                                                                           |
| R3   | 미착수               | 비교 결과를 총괄이 검토하고 제품 재작업 문서에 선택한 평가축·정확한 수식·역할/조합·동점·결측·버전을 확정한다. 후보 세 장의 실제 전후 결과를 사용자에게 제시한다.                                                                                                                                                                                                                                                    | 목표와 근거가 일치해야 한다. 거리 하한·추가 입력 등 새 제품 결정이 필요하면 그 결정만 요청한다. 미정 공식을 구현하지 않는다.                                                                                                                                                                                                                                                                                          |
