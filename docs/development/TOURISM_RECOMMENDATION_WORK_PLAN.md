...
> **현재 상태(2026-09-14): R1/R2 검토·R3 정책 확정·R4 구현 완료, R5 검증 완료·PR 인계.** 같은 브랜치에서 총괄이 단일 PR로 인계한다. 결과·검증·품질 한계는 [R5 보고서](RECOMMENDATION_QUALITY_R5_REPORT.md), 현재 제품 기준은 [추천 품질 재작업 기준](../product/RECOMMENDATION_QUALITY_RECOVERY.md)이다. 아래 T1~T7은 과거 구현 이력이다.
>
> **R2 보완(2026-09-12).** 사용자가 최초 R2를 5가지 방법론 결함(`hubRank` 지역 내 순위를 전국 순위로 오기, 허브 풀 크기 비교 없이 순위 비교, 편도2h+를 단순 부작용으로 카운트, 최종 3장 "구성" 자체를 비교하지 않음, 근거리·원거리 심층 비교 부족)으로 반려했다. 배포 코드는 이번에도 변경하지 않았고, 하네스만 확장해(§10~14) 재판정했다. 결론: 이번 R2가 시험한 대안 중 그대로 채택할 것은 없다. 대안 2는 6개 지정 지역 순위를 개선했지만 최종 승자를 바꾸지 못했고, 단일 극값과 지역 내 순위의 지역 간 비교 불가 문제를 해결하지 못해 우선 권고를 철회한다. 6개 지정 지역의 허브 풀이 대부분 100이라는 사실도 지역 간 비교 타당성을 보장하지 않는다. R3는 사용자 승인된 새 역할(가까운 여행/1박 2일 여행/관심사 중심 여행)과 이동시간 구간 정책으로 확정했으며, R4는 그 정책을 구현한다.

## 재작업 R1~R5

다음 세션의 작업자는 AGENTS.md, employee 정의, 위 제품 재작업 기준을 읽고 이 순서로 진행한다. 기존 PR #76은 닫힌 작업이다. 새 작업 브랜치를 사용하며 현재 문서 브랜치가 미병합이면 변경을 보존해 이어받는다.

| 순서 | 상태                                | 작업 및 산출물                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | 다음 단계 진입 조건                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ---- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| R1   | **완료(2026-09-11)**                | [R1 진단 보고서](RECOMMENDATION_QUALITY_R1_DIAGNOSIS.md). 고정 입력(서울·역사 단일·2026-09-12 08:00→09-13 20:00)에서 배포 코드(main, T7-6)를 그대로 실행해 전국 152개 시간 적합 후보 전부의 탈락 단계·배치 시설/유형·중심 연결·목적 적합성 구간·왕복/현지시간·역할별 순위를 추출했다(`lib/__tests__/enhancement-r1-diagnosis.test.ts` + `scripts/report-r1-diagnosis.mjs`, 재사용 가능). 코드 로직은 바꾸지 않았다.                                                                                                                                                    | **충족.** 근거리 승리 원인을 코드 경로(`compareByRole`)와 실측값으로 설명했다: 시간 적합 152개 중 139개(91%)가 동일 최고 구간(fitAverage=7=상한)에 도달해, 이 압도적 다수 안에서는 정렬 3번째 키(왕복시간)가 사실상 유일한 변별자가 된다. 공주(38위)·익산(58위)·경주(107위)는 남양주(4위)·하남(5위)과 **똑같은 최고 구간**이지만 순전히 더 멀어서 밀린다. 재작업 기준 문서의 "실패 원인" 가설과 전부 일치(보고서 §7).                                                                                                    |
| R2   | **보완·총괄 검토 완료(2026-09-14)** | [R2 대안 비교 보고서](RECOMMENDATION_QUALITY_COMPARISON.md). 기존 데이터·기존 검증 로직 위에서 4개 대안(+조합 1개)과 구성 대안 1개를 오프라인으로 비교했다(`lib/test-support/r2-comparison-core.ts` + `lib/__tests__/enhancement-r2-comparison.test.ts` + `scripts/report-r2-comparison.mjs`, 재사용 가능). 배포 코드는 전혀 수정하지 않았다. 48개 입력 매트릭스와 R1의 6개 지정 지역 전부에서 전/후 표를 만들었다.                                                                                                                                                    | **충족(진단 산출물 기준).** 대안 1(평균점수 재도입)과 대안 4/5(relaxed=최장거리)는 반대 방향 쏠림을 재현했고, 대안 3(포화 상한 상향)은 48개 매트릭스에서 승자를 바꾸지 못했다. 대안 2(단일 최고 확정 연결 순위)는 6개 지정 지역 순위를 개선했지만 최종 3곳 승자를 바꾸지 못했고, 단일 극값·허브 풀 크기 비대칭 문제를 해결하지 못했다. 역할을 없앤 단일 순위 top-3도 46/48셀이 근거리 클러스터였다. 이번 R2가 시험한 대안 중 그대로 채택할 근거는 없다. R2 종료 시점에는 R3 정책을 확정하지 않았고 R4도 착수하지 않았다. |
| R3   | **확정 완료(2026-09-14)**           | 사용자 승인에 따라 기존 `interest/easy/relaxed`를 `가까운 여행`·`1박 2일 여행`·`관심사 중심 여행`으로 교체하는 정책을 [제품 재작업 기준](../product/RECOMMENDATION_QUALITY_RECOVERY.md#r3-확정-정책-2026-09-14)에 통합했다. 편도시간 구간(<=60, 61~~180, 120~~240), 활동 조건, 역할별 동점 정렬, 중복/부족 처리, 저장 호환과 새 검색 버전(`e1-v4` 계열), 과거 R1/R2 하네스 보존 원칙을 명시했다. 서울·역사 주야간과 부산·문화 주야간 4셀을 좁게 재실측했고, 재현 명령은 `R3_OUT=/tmp/r3-role-policy-samples.json npx jest enhancement-r3-role-policy --runInBand`이다. | **R4 진입 가능.** R3 정책은 지역명 가점·최장거리 우선·새 API 없이 기존 일정 후보의 실제 시설/세부유형/시간으로 세 역할을 분리한다. 전국 대표성 개선을 입증한 것은 아니며 역할별 선택지 다양화 정책이다. `hubRank`는 지역 간 정렬 키로 쓰지 않는다. 역할 조건을 만족하지 못하면 1~2장만 반환한다. 시간 엔진 자체를 바꾸지 않으면 planner/itinerary 버전은 `e2-v3`을 유지한다. R4 구현 시 과거 R1/R2 비교 하네스와 현행 정책 검증을 분리해 이전 보고 숫자를 덮어쓰지 않는다.                                               |
| R4   | **구현 완료(2026-09-14)**           | 새 역할·시간 구간·실제 시설 조건·설명·e1-v4 및 구 저장본 호환 구현. 시간 엔진과 식당 호출 경계 유지.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 고도화 Jest 290개 및 독립 검토 통과. [R5 결과](RECOMMENDATION_QUALITY_R5_REPORT.md).                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| R5   | **검증 완료·PR 인계(2026-09-14)**   | 48개 전후 비교, 고도화 Jest 290개, 하네스·포맷·린트·타입·빌드, E2E 12개, 독립 검토 완료. [최종 보고서](RECOMMENDATION_QUALITY_R5_REPORT.md).                                                                                                                                                                                                                                                                                                                                                                                                                           | 사용자 품질 수용은 별도다. 총괄이 PR을 생성하고 병합은 이후 지시를 따른다.                                                                                                                                                                                                                                                                                                                                                                                                                                               |

R1·R2에서 실행 가능한 근거를 먼저 확보한다. API 성공, 데이터 건수 증가, 테스트 통과만으로 R3를 통과시키지 않는다. 현재 easy/relaxed 유지나 같은 등급의 최단 이동을 새 계획의 고정 요구로 가져오지 않는다. R3 이후 구현 검증은 "과거 알고리즘 진단 재현"과 "현행 정책 품질 검증"을 분리한다. 제품 수용 기준은 위 재작업 문서가 단일 출처다.

## 이전 구현 이력 — T1~T7

아래의 미착수·병합 대기·남은 작업 문구는 당시 기록이다. 현재 다음 작업으로 해석하지 않는다.
...
T1→T2→T3→T4→T5→T6→T7 구현을 완료하고 PR #76으로 병합했다. 구현 완료와 추천 품질 목표 충족은 다르며, 현재 재작업은 위 R1~R5다.

### T7: 목적 적합성 보완 — 구현·병합 완료, 품질 목표 미달
