...
> **현재 상태(2026-09-11): PR #76 병합·배포 완료, 추천 품질 목표 미달. R1·R2 완료, 다음 작업은 R3(총괄 검토·확정)다.** 아래 T1~~T7은 구현 이력이며 다시 실행할 작업이 아니다. 제품 재작업 기준은 [목적지 추천 품질 재작업 기준](../product/RECOMMENDATION_QUALITY_RECOVERY.md)이 우선한다. R1 산출물은 [R1 진단 보고서](RECOMMENDATION_QUALITY_R1_DIAGNOSIS.md), R2 산출물은 [R2 대안 비교 보고서](RECOMMENDATION_QUALITY_COMPARISON.md)다.
...
| R2   | **완료(2026-09-11)** | [R2 대안 비교 보고서](RECOMMENDATION_QUALITY_COMPARISON.md). 기존 데이터·기존 검증 로직 위에서 4개 대안(+조합 1개)을 오프라인으로 비교했다(`lib/test-support/r2-comparison-core.ts` + `lib/__tests__/enhancement-r2-comparison.test.ts` + `scripts/report-r2-comparison.mjs`, 재사용 가능). 배포 코드는 전혀 수정하지 않았다. 48개 입력 매트릭스와 R1의 6개 지정 지역 전부에서 전/후 표를 만들었다.                 | **부분 충족.** 대안 1(이전 e1-v2 원점수 재도입)은 T6의 원거리 대도시 쏠림을 재현했고(interest 왕복 중앙값 48→206분, 편도2h+ 0/48→21/48), 대안 4(relaxed=최고구간 내 최장거리)는 relaxed의 현지 자유시간을 파괴하며 근거 없는 초원거리를 골랐다(현지자유 중앙값 780→255분, 40/48셀 근거 0건). 대안 3(포화 상한 상향)은 48개 매트릭스에서 승자를 단 하나도 바꾸지 못해 명확한 개선이 없었다. 대안 2(단일 최고 확정 연결 순위, 평균·합산 없이)만 6개 지정 지역의 순위를 뚜렷이 개선하면서(익산 58→6위, 공주 38→15위, 경주 107→34위) 부작용을 대안 1의 1/3 이하로 억제했지만, 이 고정 입력에서 최종 3곳 승자를 바꾸지는 못했다. 재작업 기준의 "명확한 개선이 없으면 실패로 본다"에 따라 그대로 채택할 대안은 없으며, 대안 2를 R3 1차 검토 대상으로 권고한다. |
| R3   | 미착수               | 비교 결과를 총괄이 검토하고 제품 재작업 문서에 선택한 평가축·정확한 수식·역할/조합·동점·결측·버전을 확정한다. 후보 세 장의 실제 전후 결과를 사용자에게 제시한다.                                                                                                                                                                                                                                                    | **진입 가능(확정은 미실시).** R2가 4개 대안의 실측 비교표를 확보했으므로 총괄이 검토할 재료는 준비됐다. 다만 R2가 그대로 채택 가능한 대안을 확정하지 못했으므로, R3는 "대안 2를 어떻게 다듬을지"(R2 보고서 §8) 또는 "최종 세 장 구성 정책 자체를 바꿀지"(원거리 고신뢰 후보를 위한 새 슬롯·역할 등, 이번에는 미검증) 중 하나를 제품 판단으로 정해야 진행할 수 있다. 목표와 근거가 일치해야 한다. 거리 하한·추가 입력 등 새 제품 결정이 필요하면 그 결정만 요청한다. 미정 공식을 구현하지 않는다.                                                                                                                                                                                                                                                         |
| R4   | 미착수               | 확정된 기준을 검색·점수·후보 설명에 반영하고 필요한 저장 호환을 유지한다. 시간 엔진·식당 흐름을 보존한다.                                                                                                                                                                                                                                                                                                           | 순위 이유와 실제 배치 장소가 일치한다. 기존 데이터/지역별 하드코딩으로 결과를 만들지 않는다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| R5   | 미착수               | 고정 사례 비교 재실행, 의미 있는 회귀 검사, verify·핵심 E2E와 독립 검토 후 최종 세 카드 결과를 보고한다.                                                                                                                                                                                                                                                                                                            | 코드 검증과 제품 품질 판정을 별도로 기록한다. 사용자 결과 확인 전 해결 완료로 표시하지 않는다. PR·병합은 해당 작업의 사용자 지시에 따른다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |

R1·R2에서 실행 가능한 근거를 먼저 확보한다. API 성공, 데이터 건수 증가, 테스트 통과만으로 R3를 통과시키지 않는다. 현재 easy/relaxed 유지나 같은 등급의 최단 이동을 새 계획의 고정 요구로 가져오지 않는다. 제품 수용 기준은 위 재작업 문서가 단일 출처다.
