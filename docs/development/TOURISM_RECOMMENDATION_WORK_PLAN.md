...
> **현재 상태(2026-09-11): PR #76 병합·배포 완료, 추천 품질 목표 미달. 다음 작업은 R1이다.** 아래 T1~~T7은 구현 이력이며 다시 실행할 작업이 아니다. 제품 재작업 기준은 [목적지 추천 품질 재작업 기준](../product/RECOMMENDATION_QUALITY_RECOVERY.md)이 우선한다. 이번에는 문서만 수정했으며 R1~~R5 실행은 미착수다.
...
| ---- | ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| R1   | 미착수 · 다음 작업 | 기존 데이터·알고리즘 버전과 고정 입력을 기록하고 모든 후보의 탈락 단계, 실제 배치 시설·유형·중심 연결·구간·시간·역할 순위를 추출한다. 서울 역사 08:00→익일20:00에서 남양주·하남·과천과 공주·익산·경주를 나란히 비교한다. 사용자 관찰 입력은 미확보로 구분한다. | 왜 가까운 후보가 이겼고 다른 후보가 탈락했는지 코드 경로와 실제 값으로 설명한다. 재수집 없이 우선 진단한다.                                 |
| R2   | 미착수             | 기존 데이터로 구별 가능한 목적 적합성과 구별 불가능한 매력을 분리한다. 소수의 대안 정렬/세 후보 조합을 오프라인으로 비교한다. 결과는 `docs/development/RECOMMENDATION_QUALITY_COMPARISON.md` 하나에 입력·버전·실행 명령·전후 표·한계와 함께 작성한다.          | 기존 48개 입력 및 주요 후보의 실제 장소로 개선 근거를 확인한다. 명확한 개선이 없으면 실패를 보고하고 근거 없는 상수 튜닝을 반복하지 않는다. |
